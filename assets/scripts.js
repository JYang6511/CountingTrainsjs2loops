...
Inside your loop from step 2, check and see if the car type is equal to "S1". If it is, increment the typeS1Count variable by one.
...
Stop and check: Run your code and see what prints out. 
It should be counting three "S1" cars, so you should see a console.log statement printing "Number of S1 Cars is: 3".

Perhaps you've just learned that "T1" and "S1" types are nearly identical. 
Try changing the original array and change the "T1" types to "S1" and run the code again. Does it now print "Number of S1 Cars is: 4"?

Write another loop but this time use a for loop. 
Remember, we are just using the loops to iterate through an array. We can use either type and accomplish the same objective.

Outside of this second loop, make a variable called lorrieCount. 
